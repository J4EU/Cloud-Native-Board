  ...
  <div class="f1-admin">
    ...
    <div v-if="activeTab === 'users'" class="f1-card admin-card">
      <h2 class="section-title">
        ACTIVE DRIVERS <span class="count">[{{ users.length }}]</span>
      </h2>
    ...
    <div v-if="activeTab === 'posts'" class="f1-card admin-card">
      <h2 class="section-title">
        SESSION LOGS <span class="count">[{{ posts.length }}]</span>
      </h2>
      <div class="table-container">
        <table class="admin-table">
          <thead>
          ...
          <tbody>
            <tr v-for="post in posts" :key="post.id">
              ...
              <td class="center">
                <button
                  @click="deletePostByAdmin(post.id)"
                  class="btn-force-delete"
                >
                  DELETE
                </button>
              </td>
...
const deletePostByAdmin = async (id) => {
  if (
    confirm(
      "정말로 이 게시글을 강제 삭제하시겠습니까? 삭제 후에는 복구할 수 없습니다.",
    )
  ) {
    try {
